...
onload = function () {
    ...
    (function () {
        ...
        var viewVector = [1, 1, 1];
        var meshVector = [0, 1, 0];


        //外積で軸を求める
        var arrowVector = [
            viewVector[1] * meshVector[2] - viewVector[2] * meshVector[1],
            viewVector[2] * meshVector[0] - viewVector[0] * meshVector[2],
            viewVector[0] * meshVector[1] - viewVector[1] * meshVector[0]
        ]
        var arrowVector2 = []
        vec3.cross(arrowVector2, viewVector, meshVector);
        vec3.normalize(arrowVector2, arrowVector2)
        console.log(arrowVector2)

        var angle = Math.acos(vec3.dot(viewVector, meshVector) / (vec3.length(viewVector)*vec3.length(meshVector)))
        console.log("deg: "+ 180 / Math.PI * angle)

        q.rotate(angle, arrowVector2, xQuaternion);
        var qMatrix = m.identity(m.create());
        q.toMatIV(xQuaternion,qMatrix);
